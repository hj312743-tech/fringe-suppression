...
# ==========================================
# Physics-consistency loss within the valid region only
# ==========================================
def make_valid_mask(H: int, W: int, border: int, device: torch.device) -> torch.Tensor:
    """
    Valid-region mask:
    - reliable central region is set to 1
    - a border of width border is set to 0
    Purpose: prevent FFT circular-boundary artifacts from contributing to the loss.
    """
...
def multi_scale_masked_mse(pred: torch.Tensor, target: torch.Tensor, valid_mask: torch.Tensor) -> torch.Tensor:
    """
    Multi-scale physics consistency:
    - native resolution 1x
    - 2x downsampling
    - 4x downsampling
    Computed only inside the valid field of view.
    """
...
# ==========================================
# Network: object-only U-Net / Res-U-Net branch
# ==========================================
...
class ObjectResUNet(nn.Module):
    ...
    - obj_phase : object phase in (-pi, pi)

    Notes:
    This is the only learnable module in the U-Net-PC baseline.
    No explicit background branch, fringe branch, or support prior is introduced.
    """
...
class UNetPCModel(nn.Module):
    """
    U-Net-PC baseline:
    The U-Net outputs a complex field, which is then forward-propagated using ASM.
    The input hologram is fitted through the physics-consistency constraint.
    """
    def __init__(self, in_channels: int = 1, base_f: int = 24):
        super().__init__()
...
def reconstruct_unet_pc(
    ...
    """
    U-Net-PC baseline:
    - use fixed random noise as input
    - the network outputs obj_amp and obj_phase
    - forward ASM propagation gives I_pred
    - optimize only the physics-consistency term
    """
    ...
    # Conventional ASM baseline; used only for visualization and comparison, not for support construction.
    ...
    net = UNetPCModel(in_channels=1, base_f=base_f).to(device)
    optimizer = optim.Adam(net.parameters(), lr=lr)

    loss_history = []

    print('Start U-Net-PC iterative reconstruction...')
    for i in range(iters):
        ...
        loss_history.append(float(loss_phy.item()))

        if (i + 1) % 300 == 0:
            print(f'Iter {i + 1:04d} | Physics: {loss_phy.item():.6f}')
...
# Visualization
# ==========================================
def draw_results_unet_pc(
    intensity: np.ndarray,
    results: dict,
    save_path: str = 'PNG/unet_pc_canvas.png',
    out_dir: str = 'PNG',
    prefix: str = 'sample_01_patch_0006_unetpc',
    ...
    axs[0, 3].plot(loss_history, linewidth=1.6)
    axs[0, 3].set_title('(d) Physics Loss Curve')
    ...
    im4 = axs[1, 0].imshow(results['base_amp'], cmap='gray', vmin=bmin, vmax=bmax)
    axs[1, 0].set_title('(e) Baseline ASM (Amp)')
    plt.colorbar(im4, ax=axs[1, 0], fraction=0.046, pad=0.04)

    amin, amax = percentile_limits(obj_amp_vis, 1, 99.5)
    im5 = axs[1, 1].imshow(obj_amp_vis, cmap='gray', vmin=amin, vmax=amax)
    axs[1, 1].set_title('(f) U-Net-PC Amp')
    plt.colorbar(im5, ax=axs[1, 1], fraction=0.046, pad=0.04)

    cmin, cmax = percentile_limits(obj_contrast_vis, 1, 99.5)
    im6 = axs[1, 2].imshow(obj_contrast_vis, cmap='cividis', vmin=cmin, vmax=cmax)
    axs[1, 2].set_title('(g) U-Net-PC Contrast (1-Amp)')
    plt.colorbar(im6, ax=axs[1, 2], fraction=0.046, pad=0.04)

    im7 = axs[1, 3].imshow(obj_phase_vis, cmap='twilight_shifted', vmin=-np.pi, vmax=np.pi)
    axs[1, 3].set_title('(h) U-Net-PC Phase')
    ...
    if save_single:
        ...
        save_single_panel(results['base_amp'], f'{prefix}_e_baseline_amp.png', cmap='gray', vmin=bmin, vmax=bmax)
        save_single_panel(obj_amp_vis, f'{prefix}_f_unetpc_amp.png', cmap='gray', vmin=amin, vmax=amax)
        save_single_panel(obj_contrast_vis, f'{prefix}_g_unetpc_contrast.png', cmap='cividis', vmin=cmin, vmax=cmax)
        save_single_panel(obj_phase_vis, f'{prefix}_h_unetpc_phase.png', cmap='twilight_shifted', vmin=-np.pi, vmax=np.pi)
...
if __name__ == '__main__':
    ...
    # -------- Visualization --------
    draw_results_unet_pc(
        intensity,
        results,
        #save_path='PNG/unet_pc_recon_canvas2_patch_0006.png',
        out_dir='PNG/U-Net-PC',
        #prefix='sample_01_patch_0006_unetpc',
        save_single=True,
    )

    # -------- Evaluation interface output --------
    out_eval_dir = os.path.join('outputs', 'U-Net-PC', 'sample_007', 'patch_0013')
    os.makedirs(out_eval_dir, exist_ok=True)

    # Table 1 uses the reconstructed amplitude, keeping the same evaluation interface as Ours.
    np.save(os.path.join(out_eval_dir, 'table1_img.npy'), results['obj_amp'].astype(np.float32))

    # Save I_pred for forward-consistency checking.
    ...
    np.save(os.path.join(out_eval_dir, 'loss_history.npy'), results['loss_history'].astype(np.float32))

    print(f'U-Net-PC results saved to: {out_eval_dir}')
